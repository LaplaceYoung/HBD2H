...
const CARDS: CardData[] = cardsDataRaw as CardData[];
...
export function drawCards(spread: Spread): DrawnCard[] {
    ...
    // Pick N cards based on spread
    return spread.positions.map((pos, index) => {
        const isReversed = Math.random() > 0.6; // 40% chance of reversed
        return {
            ...shuffledCards[index],
            isReversed,
...
/**
 * Generate completion prompt for LLM based on user question, spread, and drawn cards
 */
